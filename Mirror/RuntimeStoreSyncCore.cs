...
namespace DingoGameObjectsCMS.Mirror
{
    ...
    {
        public static bool ShouldReplicateObject(GameRuntimeObject obj, ReplicationMask mask, int connectionId = -1, int replicationProfileId = 0)
        {
            if (obj == null)
                return false;
            ...
            if (!IsOwnerVisible(obj, null, connectionId))
                return false;

            return true;
        }

        public static bool ShouldReplicateComponent(GameRuntimeObject owner, GameRuntimeComponent component, ReplicationMask mask, int connectionId = -1, int replicationProfileId = 0)
        {
            if (component == null)
                return false;
            ...
            if (!IsOwnerVisible(owner, component, connectionId))
                return false;
        ...
        private static bool IsOwnerVisible(GameRuntimeObject owner, GameRuntimeComponent component, int connectionId)
        {
    ...
    public static partial class RuntimeStoreSnapshotCodec
    {
            ...
            IRuntimePayloadSerializer serializer = null)
        {
            ...
            for (var i = 0; i < roots.Count; i++)
                BuildSnapshotRecursive(store, snapshot, roots[i], parentId: -1, index: i, connectionId, replicationProfileId, serializer);
            ...
            IRuntimePayloadSerializer serializer)
        {
            ...
            if (!RuntimeReplicationFilter.ShouldReplicateObject(obj, ReplicationMask.Snapshot | ReplicationMask.Delta, connectionId, replicationProfileId))
                return;
            ...
            for (var i = 0; i < children.Count; i++)
                BuildSnapshotRecursive(store, snapshot, children[i], id, i, connectionId, replicationProfileId, serializer);
        }
