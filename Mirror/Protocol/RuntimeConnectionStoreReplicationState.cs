...
namespace DingoGameObjectsCMS.Mirror.Protocol
{
    ...
    public class RuntimeConnectionStoreReplicationState
    {
        ...
        public RuntimeConnectionStoreReplicationState(NetStoreRef store, ulong initialStoreRevision, int maxPendingEnvelopeCount = RuntimeProtocol.MAX_PENDING_ENVELOPES, int maxPendingDeltaBytes = RuntimeProtocol.MAX_PENDING_ENVELOPE_BYTES)
        {
            if (!store.IsValid)
