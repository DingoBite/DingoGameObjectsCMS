...
namespace DingoGameObjectsCMS.Mirror.Protocol
{
    ...
    public sealed class RuntimeBaselineChunkAssembler
    {
        ...
        public RuntimeBaselineChunkResult Accept(in RuntimeBaselineChunk chunk, double nowSeconds, out byte[] completedPayload)
        {
            ...
            if (_chunks != null && nowSeconds - _startedAt > RuntimeProtocol.BASELINE_TIMEOUT_SECONDS)
            {
                ResetActive();
        ...
        private static bool ValidateHeader(in RuntimeBaselineChunk chunk)
        {
            ...
            if (!HasValidCheckpointMetadata(chunk))
                return false;
            if (chunk.ChunkCount == 0 || chunk.ChunkCount > RuntimeProtocol.MAX_BASELINE_CHUNKS || chunk.ChunkIndex >= chunk.ChunkCount)
                return false;
            if (chunk.LogicalLength < 0 || chunk.LogicalLength > RuntimeProtocol.MAX_BASELINE_BYTES)
                return false;
