...
namespace DingoGameObjectsCMS.Mirror.Protocol
{
    ...
    public class RuntimeSessionManifestTemplate
    {
        ...
        {
            if (descriptor.ProtocolVersion != RuntimeProtocol.VERSION)
            {
                throw new InvalidOperationException(
                    $"Local protocol version {descriptor.ProtocolVersion} does not match required version {RuntimeProtocol.VERSION}.");
            }
            ...
            if (!RuntimeSessionManifestSnapshot.TryCreate(
                    ...
                    out var error))
            {
                throw new InvalidOperationException($"Invalid local protocol manifest: {error.Detail}");
            }
        ...
        public RuntimeSessionManifestSnapshot CreateSnapshot(ulong sessionId)
        {
            if (!RuntimeSessionManifestSnapshot.TryCreate(
                    ...
                    out var error))
            {
                throw new InvalidOperationException($"Unable to create protocol session manifest: {error.Detail}");
            }
    ...
    public class RuntimeSessionManifestSnapshot
    {
        ...
        public RuntimeSessionHandshakeResult ValidateAgainst(RuntimeSessionManifestTemplate expected)
        {
            ...
            var validation = RuntimeSessionDescriptorValidator.Validate(expected.Descriptor, Descriptor);
    ...
    public class RuntimeSessionClientExpectation
    {
        ...
        {
            if (descriptor.ProtocolVersion != RuntimeProtocol.VERSION)
                throw new InvalidOperationException($"Client protocol version must be {RuntimeProtocol.VERSION}.");
            if (string.IsNullOrWhiteSpace(descriptor.BuildId)
                || string.IsNullOrWhiteSpace(descriptor.RuntimeSchemaHash)
                || string.IsNullOrWhiteSpace(descriptor.AssetCatalogHash)
                || string.IsNullOrWhiteSpace(descriptor.StateStreamCatalogHash))
            {
                throw new InvalidOperationException("Client protocol descriptor is incomplete.");
            }
            if (assets == null)
            ...
            if (_storeIds.Length == 0 || _storeIds.Distinct(StringComparer.Ordinal).Count() != _storeIds.Length)
                throw new InvalidOperationException("Client protocol expectation requires unique StoreIds.");
            ...
            if (!string.Equals(assetHash, descriptor.AssetCatalogHash, StringComparison.Ordinal))
                throw new InvalidOperationException("Client protocol asset catalog does not match its descriptor hash.");
    ...
    public class RuntimeSessionServerHandshake
    {
        ...
        public RuntimeSessionHandshakeResult ReceiveHello(in RuntimeSessionDescriptor descriptor, ulong clientNonce)
        {
            ...
            var validation = RuntimeSessionDescriptorValidator.Validate(_manifestTemplate.Descriptor, descriptor);
            if (validation != RuntimeProtocolRejectCode.None)
            {
                return Reject(validation, $"Hello descriptor mismatch: {validation}.");
            }
        ...
        public RuntimeSessionHandshakeResult AuthorizeStoreAccess(ulong sessionId, in NetStoreRef store)
        {
            ...
            {
                return RuntimeSessionHandshakeResult.Reject(RuntimeProtocolRejectCode.SessionNotReady, "Session has not completed the protocol handshake.");
            }
    ...
    public class RuntimeSessionClientHandshake
    {
            ...
            IEnumerable<RuntimeStoreCatalogEntry> stores)
        {
            ...
            var validation = _expectation.Validate(manifest);
            if (!validation.Accepted)
            {
                return Reject(validation.RejectCode, validation.Detail);
            }
