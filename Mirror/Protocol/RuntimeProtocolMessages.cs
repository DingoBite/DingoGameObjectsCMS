...
namespace DingoGameObjectsCMS.Mirror.Protocol
{
    ...
    public static class RuntimeReliableDeltaTransportBudget
    {
            ...
            int reliableTransportPacketBytes)
        {
            ...
            var protocolMaxPackedBytes = RuntimeProtocol.MAX_RELIABLE_DELTA_BATCH_BYTES
                                         - Batcher.MaxMessageOverhead(RuntimeProtocol.MAX_RELIABLE_DELTA_BATCH_BYTES);
        ...
        private static int ResolveReliableTransportPacketBytes()
        {
            ...
            return transport == null
                ? RuntimeProtocol.MAX_RELIABLE_DELTA_BATCH_BYTES
    ...
    public static class RuntimeCommandJournalWireCodec
    {
        public static RtCommandJournalBatch ToWire(RuntimeCommandJournalBatch batch)
        {
            ...
            if (batch.Entries.Count > RuntimeProtocol.MAX_JOURNAL_BATCH_ENTRIES)
            {
                throw new InvalidOperationException(
                    $"Journal batch contains {batch.Entries.Count} entries; limit is {RuntimeProtocol.MAX_JOURNAL_BATCH_ENTRIES}.");
            }
            ...
            if (payloadBytes > RuntimeProtocol.MAX_JOURNAL_BATCH_BYTES)
            {
                throw new InvalidOperationException(
                    $"Journal batch payload is {payloadBytes} bytes; limit is {RuntimeProtocol.MAX_JOURNAL_BATCH_BYTES}.");
            }
        ...
        public static RuntimeCommandJournalBatch FromWire(in RtCommandJournalBatch wire)
        {
            ...
            if (wireEntries.Length > RuntimeProtocol.MAX_JOURNAL_BATCH_ENTRIES)
            {
                throw new FormatException(
                    $"Journal batch contains {wireEntries.Length} entries; limit is {RuntimeProtocol.MAX_JOURNAL_BATCH_ENTRIES}.");
            }
